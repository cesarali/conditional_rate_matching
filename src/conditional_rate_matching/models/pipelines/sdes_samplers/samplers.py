...
def ConditionalTauLeaping(config:Union[DSBConfig,CTDDConfig,CRMConfig],
                          rate_model:Union[ClassificationForwardRate,SchrodingerBridgeRate],
    ...
    number_of_paths = x_start.size(0)
    D = x_start.size(1)
    S = config.data0.vocab_size
    num_steps = config.pipeline.number_of_steps
    config.data1.conditional_dimension
    min_t = 1./num_steps
    device = x_start.device

    x_condition = x_start[:,:config.data1.conditional_dimension]
    h_start = rate_model.conditional_layer(x_condition)
    x_0 = x_start[:,config.data1.conditional_dimension:]
