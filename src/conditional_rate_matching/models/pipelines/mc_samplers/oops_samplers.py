...
class DiffSampler(nn.Module):
    def __init__(self, config:OopsConfig):
        super().__init__()
        self.dim = config.data0.dimensions
        self.n_steps = config.sampler.n_steps
        self._ar = 0.
        self._mt = 0.
        self._pt = 0.
        self._hops = 0.
        self._phops = 0.
        self.approx = config.sampler.approx
        self.fixed_proposal = config.sampler.fixed_proposal
        self.multi_hop = config.sampler.multi_hop
        self.temp = config.sampler.temp
        self.step_size = config.sampler.step_size
        if config.sampler.approx:
...
class PerDimGibbsSampler(nn.Module):
    def __init__(self, config:OopsConfig):
        dim = config.data0.dimensions
        rand = config.sampler.rand
        super().__init__()
        self.dim = dim
