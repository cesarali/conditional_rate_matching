...
class TestSBTrainer(unittest.TestCase):
    ...
    def test_load_and_plot(self):
        ...
        sb = SB()
        results_,all_metrics,device = sb.load_from_results_folder(experiment_name="graph",
                                                                  experiment_type="sb",
        ...
        paths, times = sb.pipeline(past_model,sinkhorn_iteration=1, device=device, return_path=True)
        loss = sb.backward_ratio_estimator(current_model,past_model,paths,times)
    ...
    def test_restart_training(self):
        sb_trainer = SBTrainer(config=None,
                               ...
                               experiment_type="sb",
                               experiment_indentifier="community_small_to_bernoulli",
                               new_experiment_indentifier="community_small_to_bernoulli_2",
                               sinkhorn_iteration_to_load=0,
                               next_sinkhorn=True)
