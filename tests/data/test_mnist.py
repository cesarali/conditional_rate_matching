...
class TestDataLoader(unittest.TestCase):
    ...
    def test_nist(self):
        data_config = NISTLoaderConfig(as_image=False,flatten=False)
        data_loader = NISTLoader(data_config)

        databatch = next(data_loader.train().__iter__())

        print(f"databatch_shape {databatch[1].shape} expected shape {data_config.temporal_net_expected_shape} expected dim {data_config.dimensions}")
